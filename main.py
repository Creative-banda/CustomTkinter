...
class WordverseGame:
    ...
    def setup_word_match_interface(self):
        ...
        new_round_btn = ctk.CTkButton( control_frame,  text="🎲 New Round",  command=self.reset_word_match, font=(self.font, 18, "bold"),  
    ...
    def create_match_words(self):
        ...
        self.words_grid = ctk.CTkFrame(self.words_match_frame)
        self.words_grid.pack(pady=10)
        
        for i, (word, category) in enumerate(all_words):
            btn = ctk.CTkButton( self.words_grid, text=word, command=lambda w=word, c=category: self.select_word_for_match(w, c), 
    ...
    def remove_word_from_available(self, word_to_remove):
        ...
        for widget in self.words_grid.winfo_children():
            if isinstance(widget, ctk.CTkButton) and widget.cget("text") == word_to_remove:
                widget.destroy()
                break
